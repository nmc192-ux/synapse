...
from loop_planner import write_loop_status_artifacts, write_next_iteration_artifacts
...
def build_daily_report(projects: list[dict[str, object]], summary: dict[str, object]) -> str:
    ...
    lines = [
        ...
        *[f"  - {reason}" for reason in alpha_gate["reasons"]],
        ...
        f"- Runs timed out in operator review: {summary.get('timed_out_operator_review_runs', 0)}",
        ...
        f"- Pending operator-review interventions timed out: {summary.get('timed_out_operator_review_interventions', 0)}",
        "",
        ...
        f"- Operator-review timed out in harness: {summary['request_health_summary']['operator_review_timed_out']}",
    ...
    for project in projects:
        ...
        lines.extend(
            [
                ...
                f"- Runs timed out in operator review: {project.get('timed_out_operator_review_runs', 0)}",
                ...
                f"- Pending operator-review interventions timed out: {project.get('timed_out_operator_review_interventions', 0)}",
                f"- Failure rate: {project['per_project_failure_rate']:.2%}",
                f"- Request health slow/stuck/recovered/abandoned/operator-required/operator-review-overdue/operator-review-timed-out/completed-after-slow/unresolved: {project['request_health_summary']['slow']}/{project['request_health_summary']['stuck']}/{project['request_health_summary']['recovered']}/{project['request_health_summary']['abandoned']}/{project['request_health_summary']['operator_required']}/{project['request_health_summary']['operator_review_overdue']}/{project['request_health_summary']['operator_review_timed_out']}/{project['request_health_summary']['completed_after_slow']}/{project['request_health_summary']['unresolved']}",
...
def build_weekly_review(projects: list[dict[str, object]], summary: dict[str, object]) -> str:
    ...
    lines = [
        ...
        *[f"- {agent_id}: {count} interventions" for agent_id, count in intervention_heavy],
        ...
        f"- Operator-review timed out: {summary['request_health_summary']['operator_review_timed_out']}",
        f"- Completed after slow/stuck: {summary['request_health_summary']['completed_after_slow']}",
...
def build_dashboard_html(daily_summary: dict[str, object], weekly_summary: dict[str, object]) -> str:
    ...
    return (
        ...
        f"<section class='card'><h2>Daily Summary</h2><p>Runs started: {daily_summary['runs_started']}</p><p>Runs failed: {daily_summary['runs_failed']}</p><p>A2A failures: {daily_summary['a2a_messages_failed']}</p><p>Avg latency: {daily_summary['average_run_latency_seconds']}s</p><p>Waiting for operator: {daily_summary.get('waiting_for_operator_runs', 0)}</p><p>Waiting beyond timeout: {daily_summary.get('stale_waiting_for_operator_runs', 0)}</p><p>Timed out in review: {daily_summary.get('timed_out_operator_review_runs', 0)}</p><p>Pending operator review: {daily_summary.get('pending_operator_review_interventions', 0)}</p><p>Overdue operator review: {daily_summary.get('overdue_operator_review_interventions', 0)}</p><p>Timed-out interventions: {daily_summary.get('timed_out_operator_review_interventions', 0)}</p></section>"
...
def fixture_project_summary(project_alias: str, started: int, completed: int, failed: int, interventions: int) -> dict[str, object]:
    snapshot = {
        ...
        "timed_out_operator_review_runs": 0 if project_alias == "steady" else 1,
        ...
        "timed_out_operator_review_interventions": 0 if project_alias == "steady" else 1,
        ...
        "request_health_summary": {
            ...
            "operator_review_timed_out": 0 if project_alias == "steady" else 1,
...
def run_once() -> None:
    ...
    loop_status_paths = write_loop_status_artifacts(
        {"daily": {"projects": daily_projects, "summary": daily_summary}, "weekly": {"projects": weekly_projects, "summary": weekly_summary}}
    )
    print(
        {
